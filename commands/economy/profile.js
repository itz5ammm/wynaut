...
Canvas.registerFont(
  `${process.cwd()}/assets/fonts/RobotoRegular.ttf`,
  "RobotoRegular"
);
...
module.exports = {
  ...
  execute: async (bot, message, args) => {
    let user =
      ...
      message.member;
    if (user.user.bot)
      return message.channel.send(`**Bots Don't Have A Profile!**`);
    ...
    if (bg === null)
      bg =
        "https://images.wallpaperscraft.com/image/cat_profile_muzzle_eyes_113734_3840x2400.jpg";
    ...
    try {
      async function createCanvas() {
        ...
        var name =
          username.length > 10 ? username.substring(0, 12) + "..." : username;
        var { body: avatar } = await get(
          user.user.displayAvatarURL({ format: "jpg", size: 1024 })
        );
        var { body: background1 } = await get(background);
        var { body: background2 } = await get(
          "https://cdn.glitch.com/ccfc9e2e-e1fa-4dd0-838d-c3b5bb122b10%2Fprofile.png?v=1589468840464"
        );
      ...
      return message.channel.send(
        `Oh no an error occurred :( \`${e.message}\` try again later.`
      );
    }
  }
};
